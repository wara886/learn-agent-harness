...
当前 beta 包含三课 DSH 入门与三课 Pi 对照：工具结果如何改变回答、Session 记录如何投影成模型消息、两套 harness 如何扩展工具能力，以及 Pi 如何转换应用消息并把工具结果送回下一轮。演示全部在浏览器本地确定性运行，不调用模型、不需要 API key，也不产生 API 费用。
...
- 六课共享一套显式状态机：预测、运行、观察、单变量实验、迁移检查。
