...
当前 beta 包含三课 DSH 入门与一课 Pi 对照：工具结果如何改变回答、Session 记录如何投影成模型消息、工具能力如何注册并撤销，以及 Pi 如何把工具结果送回下一轮。演示全部在浏览器本地确定性运行，不调用模型、不需要 API key，也不产生 API 费用。
...
- 四课共享一套显式状态机：预测、运行、观察、单变量实验、迁移检查。
- 桌面固定课程目录、移动端折叠目录、当前位置面包屑、问题语言搜索、本地进度恢复、单课重置和全部清除。
...
当前三课中的 DeepSeek Harness 产品事实固定到 `b150a551b8d465e31e418e1b2eaf5e79bbb7d28e`，Pi 对照课固定到 `earendil-works/pi@c49906ec77788625aacbdc53ebca6fbe65bd20f5`。教学状态机是概念演示，不是两个上游项目的 API，也不保证真实模型采取相同决策。
