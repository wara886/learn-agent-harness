...
实施前提供一个方向稿，只确认视觉世界，不先实现页面。建议方向是“安静的执行工作台”：高对比中性色作为阅读底，动作、成功、判断和错误使用功能色；视觉焦点是轨迹与结果，不使用渐变、装饰光球、插画 hero、过量阴影或一屏多色课程卡。

具体配色、字体、品牌符号和动效节奏必须等待用户确认。确认前不把审美选择写成实现事实。
...
M4 的 5 人测试达到 `REQUIREMENTS.md` 指标，三课共用同一内容模型，没有事实错误或特殊状态机，并且主要反馈已经从“不知道从哪开始/看不进去”转为具体课程问题。
